...
get_google_buildings<-function(feature=NULL,output.format="csv"){
  if(output.format%in%c("csv","sf")==FALSE){
    stop('Parameter output.format it must be "csv" or "sf"')
  }
  ...
  for (i in 1:nrow(filter)) {
    cat(paste0("Downloading tile ",i," of ",nrow(filter),"  Size: ",filter$size_mb[i],"mb"))
